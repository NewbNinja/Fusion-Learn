...
public class NetworkCharacterControllerPrototypeCustom : NetworkTransform
{
    ...
    public virtual void Move(Vector3 direction)
    {
        ...
        if (direction == default){ horizontalVel = Vector3.Lerp(horizontalVel, default, braking * deltaTime); }
        else
        {
            horizontalVel = Vector3.ClampMagnitude(horizontalVel + direction * acceleration * deltaTime, maxSpeed);
            // This makes the camera look in the direction of the rotation - For FPS this is not what we want (in SG we use TOP DOWN cams so this isn't relevant)
    ...
    public virtual void Rotate(float rotationY)
    {
